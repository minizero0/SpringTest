...
public interface BoardDAO extends JpaRepository<Board, Integer> {
	...
	@Query(value = "select no,b_level,b_ref,b_step,content,fname,hit,ip,pwd,regdate,title,writer,id from (select a.*, rownum n from (select * from Board where id = ?3 order by b_ref desc, b_step) a) where n between ?1 and ?2", nativeQuery=true)
